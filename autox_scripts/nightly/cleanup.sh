...
ALL_OPERATORS="authorino serverless servicemesh clusterobservability tempo opentelemetry kueue cma certmanager connectivitylink leaderworkerset jobset mariadb"
...
REPO_NAME="${REPO_NAME:-odh-gitops}"
...
Usage: $0 -t <operator|addon|helm|gitops-cli-dependencies> [-g] [-k] [-K] [-r] [-b] [-l]

  -t <type>              The type of the installation used for RHOAI when installed.
                         Valid types: operator, addon, helm, gitops-cli-dependencies
...
while getopts ":t:gkKa:r:b:l" o; do
  case "${o}" in
    t)
      ...
      if [[ ! ${INSTALLATION_TYPE} =~ ^(operator|addon|helm|gitops-cli-dependencies)$ ]]; then
        usage
      fi
      ;;
    g)
...
function delete_crds (){
  ...
  oc delete crd llminferenceservices.serving.kserve.io --ignore-not-found
  ...
  oc delete crd accounts.nim.opendatahub.io --ignore-not-found
  ...
  oc delete crd modelmeshservings.components.platform.opendatahub.io --ignore-not-found
  ...
  delete_resources rayclusters.ray.io delete_finalizers
  oc delete crd rayclusters.ray.io --ignore-not-found
  delete_finalizers_using_namespace rayjobs.ray.io
  delete_resources rayjobs.ray.io delete_finalizers
  oc delete crd rayjobs.ray.io --ignore-not-found
  delete_finalizers_using_namespace rayservices.ray.io
  delete_resources rayservices.ray.io delete_finalizers
  ...
  oc delete crd trainers.components.platform.opendatahub.io --ignore-not-found

  # Note: JobSet CRDs are deleted via cleanup_jobset() when using -a jobset
  # Note: Kueue CRDs are deleted via cleanup_kueue() when using -a kueue
  ...
  # Delete the SPARKOPERATOR CRDs
  oc delete crd sparkoperators.components.platform.opendatahub.io --ignore-not-found
  oc delete crd scheduledsparkapplications.sparkoperator.k8s.io --ignore-not-found
  oc delete crd sparkconnects.sparkoperator.k8s.io --ignore-not-found
  oc delete crd sparkapplications.sparkoperator.k8s.io --ignore-not-found
...
function cleanup_servicemesh3() {
  ...
  delete_resources "gateways.gateway.networking.k8s.io"
  delete_resources "gatewayclasses.gateway.networking.k8s.io"

  # Delete any Istio resources before removing the operator
  echo "Deleting Istio resources..."
  delete_resources "istios.sailoperator.io"
  delete_resources "istiorevisions.sailoperator.io"
  delete_resources "istiorevisiontags.sailoperator.io"
  delete_resources "istiocnis.sailoperator.io"
  delete_resources "ztunnels.sailoperator.io"

  # Clean up OLM resources
  ...
  delete_olm_resources "openshift-operators" "servicemeshoperator3" "servicemeshoperator3"
  delete_marketplace_resources "istio-sail-operator-bundle"
  ...
  oc delete crd istios.sailoperator.io --ignore-not-found
  oc delete crd istiorevisions.sailoperator.io --ignore-not-found
  oc delete crd istiorevisiontags.sailoperator.io --ignore-not-found
  oc delete crd istiocnis.sailoperator.io --ignore-not-found
  oc delete crd ztunnels.sailoperator.io --ignore-not-found
...
function cleanup_connectivitylink() {
  ...
  delete_marketplace_resources "authorino-operator-bundle"
  delete_marketplace_resources "rhcl-operator-bundle"
...
function cleanup_leaderworkerset() {
  ...
  delete_marketplace_resources "lws-operator-bundle"
...
function cleanup_certmanager() {
  ...
  delete_marketplace_resources "cert-manager-operator-bundle"
...
function cleanup_jobset() {
  # Delete JobSet resources before operator
  delete_finalizers_using_namespace jobsets.jobset.x-k8s.io
  delete_resources "jobsets.jobset.x-k8s.io"
  ...
  oc delete crd jobsets.jobset.x-k8s.io --ignore-not-found
  oc delete crd jobsetoperators.operator.openshift.io --ignore-not-found
}

# MariaDB is used by Model Explainability tests
function cleanup_mariadb() {
  delete_resources "mariadbs.k8s.mariadb.com"
  delete_resources "backups.k8s.mariadb.com"
  delete_resources "restores.k8s.mariadb.com"
  delete_resources "connections.k8s.mariadb.com"
  delete_resources "databases.k8s.mariadb.com"
  delete_resources "grants.k8s.mariadb.com"
  delete_resources "users.k8s.mariadb.com"
  delete_resources "sqljobs.k8s.mariadb.com"
  delete_resources "maxscales.k8s.mariadb.com"

  delete_olm_resources "openshift-operators" "mariadb-operator" "mariadb-operator"
  delete_marketplace_resources "mariadb-operator-bundle"

  delete_webhooks "mariadb"

  oc delete crd mariadbs.k8s.mariadb.com --ignore-not-found
  oc delete crd backups.k8s.mariadb.com --ignore-not-found
  oc delete crd restores.k8s.mariadb.com --ignore-not-found
  oc delete crd connections.k8s.mariadb.com --ignore-not-found
  oc delete crd databases.k8s.mariadb.com --ignore-not-found
  oc delete crd grants.k8s.mariadb.com --ignore-not-found
  oc delete crd users.k8s.mariadb.com --ignore-not-found
  oc delete crd sqljobs.k8s.mariadb.com --ignore-not-found
  oc delete crd maxscales.k8s.mariadb.com --ignore-not-found
}
...
if [ ${INSTALLATION_TYPE} == "operator" ]; then
    ...
    if [ -z "${ADDITIONAL_OPERATORS}" ]; then
      ADDITIONAL_OPERATORS="${ALL_OPERATORS}"
    fi

    if [ ${LOCAL_GITOPS_CLI_REPO} == "false" ]; then
      git_clone "${GITOPS_CLI_REPO_URL}" "${GITOPS_CLI_REPO_BRANCH}"
    fi
    remove_all_dependencies
elif [ ${INSTALLATION_TYPE} == "helm" ]; then
    echo "Uninstalling Helm installation..."
    if [ ${LOCAL_GITOPS_CLI_REPO} == "false" ]; then
      git_clone "${GITOPS_CLI_REPO_URL}" "${GITOPS_CLI_REPO_BRANCH}"
    fi
    helm_uninstall "${HELM_RELEASE}" "${HELM_NAMESPACE}" "${REPO_NAME}"
fi
...
if [ ${KEEP_USER_RESOURCES} = "true" ]; then
  ...
  # Delete webhook first - prevents hanging when operator pods are gone
  echo "Deleting webhooks to unblock API operations..."
  delete_webhooks "validator.trainer.kubeflow.org"
  delete_webhooks "kubeflow-validator.training-operator.kubeflow.org"
  delete_webhooks "jobset.*webhook"
  ...
  delete_resources LLMInferenceService delete_finalizers
  ...
  # Trainer v2 resources - delete in order: TrainJobs -> JobSets -> TrainingRuntimes -> ClusterTrainingRuntimes
  delete_finalizers_using_namespace trainjobs.trainer.kubeflow.org
  delete_resources trainjobs.trainer.kubeflow.org

  delete_finalizers_using_namespace jobsets.jobset.x-k8s.io
  delete_resources jobsets.jobset.x-k8s.io

  delete_finalizers_using_namespace trainingruntimes.trainer.kubeflow.org
  delete_resources trainingruntimes.trainer.kubeflow.org

  # ClusterTrainingRuntimes are cluster-scoped (use delete_finalizers, not delete_finalizers_using_namespace)
  delete_finalizers clustertrainingruntimes.trainer.kubeflow.org
  oc delete clustertrainingruntimes.trainer.kubeflow.org --all --ignore-not-found --timeout=60s || true
  ...
  # Notebooks
  delete_finalizers_using_namespace notebooks.kubeflow.org
  delete_resources notebooks.kubeflow.org
  ...
  delete_resources sparkoperators.components.platform.opendatahub.io
  delete_resources scheduledsparkapplications.sparkoperator.k8s.io
  delete_resources sparkconnects.sparkoperator.k8s.io
  delete_resources sparkapplications.sparkoperator.k8s.io
...
for operator in ${ADDITIONAL_OPERATORS}; do
  case "${operator}" in
    ...
    "jobset")
      ...
      ;;
    "mariadb")
      echo "Cleanup additional operator mariadb"
      cleanup_mariadb
      ;;
    *)
